...
pso_location::pso_location()
    : particle_numble_(500),
      feature_point_number_(50),
      variance_of_lidar_point_(1),
      search_scope_(10),
      iterations_(50),
      read_lidar_flag_(0) {
    ...
    // 两个订阅
    sub_lidar_ = n_.subscribe("/robot0/laser_0", 2, &pso_location::lidarCB, this);// 雷达话题
    sub_map_ = n_.subscribe("/amcl/map", 2, &pso_location::mapCB, this);//地图话题
    // 发布两个话题，一个是粒子云，另一个是找到的位姿
    pub_ori_ = n_.advertise<geometry_msgs::PoseWithCovarianceStamped>(
        "/initialpose", 1);
    pub_marker_ = n_.advertise<geometry_msgs::PoseArray>("/watch_point", 2);
    /*  下面这种类里面调用pthread_creat的方法可以学习
        此外，c++11有新的thread类 */
    PthreadStart();
    ros::spin();
}
...
void pso_location::dynamicCB(global_location::test1Config& config,
                             uint32_t level) {
    ROS_INFO("reconfigure request:%d,%d,%f", config.particle_number,
             config.feature_point_number, config.variance_of_lidar_point);
...
void pso_location::startGlobalLocation() {
    ...
    productParticleSwarm();//随机在地图空白处产生粒子
    calculateParticleWeight();//计算粒子的权重
    for (int i = 0; i < iterations_; i++) {
        resampleParticleSwarm();//循环对粒子重采样
        // productParticleSwarm();
        calculateParticleWeight();
    }
    cout << "end the global location." << endl;
    sort(pose_swarm_.begin(), pose_swarm_.end());
    cout << "the pose is:(" << max_weight_pose_.point.x << ","
         << max_weight_pose_.point.y << "," << max_weight_pose_.point.theta
         << ")" << max_weight_pose_.weight << endl;
    initialPose();
}

void pso_location::productParticleSwarm() {
    cout << "product the particle swarm." << endl;
    pose_swarm_.clear();//一个vector，成员包含了粒子的位置和权重
    max_weight_pose_.weight = 0;
    for (int i = 0; i < particle_numble_; i++) {
        particle pose;
        pose.point.x = rand() % (int)(global_map_.info.width *
                                      global_map_.info.resolution) +
                       rand() % 100 / 100.0;
        pose.point.y = rand() % (int)(global_map_.info.height *
                                      global_map_.info.resolution) +
                       rand() % 100 / 100.0;
        pose.point.theta =
            (rand() % 360 + rand() % 100 / 100.0) * 3.14159265 / 180.0;
        pose.weight = 0;
        if (global_map_.data[(int)(pose.point.x / global_map_.info.resolution) +
                             (int)(pose.point.y / global_map_.info.resolution) *
                                 global_map_.info.width] != 0) {
            i--;
        } else {
            pose_swarm_.push_back(pose);
        }
    }
    displayEstimatepose();// 发布/watch_point话题显示粒子群
}
...
void pso_location::resampleParticleSwarm() {
    ...
    if (pose_swarm_.size() < particle_numble_) {
        ...
        pose_swarm_.erase(
            pose_swarm_.begin(),
            pose_swarm_.begin() + pose_swarm_.size() - particle_numble_);
        cout << "descrease the particle." << endl;
    }
    for (int i = 0; i < pose_swarm_.size() * 3 / 4; i++) {
        pose_swarm_[i].point.x = rand() % (int)(global_map_.info.width *
                                                global_map_.info.resolution) +
                                 rand() % 100 / 100.0;
        pose_swarm_[i].point.y = rand() % (int)(global_map_.info.height *
                                                global_map_.info.resolution) +
                                 rand() % 100 / 100.0;
        pose_swarm_[i].point.theta =
            (rand() % 360 + rand() % 100 / 100.0) * 3.14159265 / 180.0;
        pose_swarm_[i].weight = 0;
        if (global_map_.data
                [(int)(pose_swarm_[i].point.x / global_map_.info.resolution) +
                 (int)(pose_swarm_[i].point.y / global_map_.info.resolution) *
                     global_map_.info.width] != 0) {
            i--;
        }
    }
    for (int i = pose_swarm_.size() * 3 / 4; i < pose_swarm_.size() * 7 / 8;
         i++) {
        pose_swarm_[i].point.x =
            pose_swarm_[i].point.x + (rand() % 100 - 50) / 100.0;
        pose_swarm_[i].point.y =
            pose_swarm_[i].point.y + (rand() % 100 - 50) / 100.0;
        pose_swarm_[i].point.theta =
            pose_swarm_[i].point.theta + (rand() % 100 - 50) / 100.0;
        pose_swarm_[i].weight = 0;
    }
    for (int i = pose_swarm_.size() * 7 / 8; i < pose_swarm_.size(); i++) {
        pose_swarm_[i].point.x =
            max_weight_pose_.point.x + (rand() % 100 - 50) / 100.0;
        pose_swarm_[i].point.y =
            max_weight_pose_.point.y + (rand() % 100 - 50) / 100.0;
        pose_swarm_[i].point.theta =
            max_weight_pose_.point.theta + (rand() % 100 - 50) / 100.0;
        pose_swarm_[i].weight = 0;
    }
...
void pso_location::calculateParticleWeight() {
    ...
    for (int n = 0; n < particle_numble_; n++) {
        ...
        for (int i = 0; i < watch_point_.size(); i++) {
            double map_x = cos(pose_swarm_[n].point.theta) * watch_point_[i].x -
                           sin(pose_swarm_[n].point.theta) * watch_point_[i].y +
                           pose_swarm_[n].point.x;
            double map_y = cos(pose_swarm_[n].point.theta) * watch_point_[i].y +
                           sin(pose_swarm_[n].point.theta) * watch_point_[i].x +
                           pose_swarm_[n].point.y;
                        //    粒子到障碍物的距离
            double d = distanceToObstacle(map_x, map_y);
            if (d == 100) {
                pose_swarm_[n].weight = pose_swarm_[n].weight - 1;
            } else {
                pose_swarm_[n].weight +=
                    exp((0 - d) / variance_of_lidar_point_);
            }
        }
    ...
    cout << "the best pose is:(" << max_weight_pose_.point.x << ","
         << max_weight_pose_.point.y << "," << max_weight_pose_.point.theta
         << ")" << max_weight_pose_.weight << endl;
}
...
double pso_location::distanceToObstacle(double x, double y) {
    ...
    for (int i = 0; i < search_scope_; i++) {
        for (int j = 0 - i; j < i + 1; j++) {
            if ((width + j >= 0) && (width + j < global_map_.info.width) &&
                (height + i >= 0) && (height + i < global_map_.info.height)) {
                if (global_map_.data[(height + i) * global_map_.info.width +
                                     width + j] == 100) {
                    obs_x = width + j;
                    obs_y = height + i;
                    return sqrt((obs_x * global_map_.info.resolution - x) *
                                    (obs_x * global_map_.info.resolution - x) +
                                (obs_y * global_map_.info.resolution - y) *
                                    (obs_y * global_map_.info.resolution - y));
                    break;
                }
            }
        ...
        for (int j = 0 - i; j < i + 1; j++) {
            if ((width + i >= 0) && (width + i < global_map_.info.width) &&
                (height + j >= 0) && (height + j < global_map_.info.height)) {
                if (global_map_.data[(height + j) * global_map_.info.width +
                                     width + i] == 100) {
                    obs_x = width + i;
                    obs_y = height + j;
                    return sqrt((obs_x * global_map_.info.resolution - x) *
...
// 把this指针传给了线程函数，这样线程函数可以访问类里面的成员
...
void pso_location::initialPose() {
    ...
    cout << "Had set the initial position(" << x << "," << y << "," << oz << ","
         << ow << ")" << endl;
}
...
void pso_location::singleParticleTest(double x, double y, double theta) {
    ...
    for (int i = 0; i < watch_point_.size(); i++) {
        double map_x = cos(theta) * watch_point_[i].x -
                       sin(theta) * watch_point_[i].y + x -
                       global_map_.info.origin.position.x;
        double map_y = cos(theta) * watch_point_[i].y +
                       sin(theta) * watch_point_[i].x + y -
                       global_map_.info.origin.position.y;
        double d = distanceToObstacle(map_x, map_y);
        if (d == 100.0)
            weight = weight - 1;
        else
            weight += exp((0 - d) / variance_of_lidar_point_);
    ...
    cout << "the single particle weight is: " << max_weight_pose_.weight
         << endl;
}
...
void pso_location::displayEstimatepose() {
    ...
    //     double x = cos0 * watch_point_[i].x - sin0 * watch_point_[i].y +
    //     max_weight_pose_.point.x + global_map_.info.origin.position.x; double
    //     y = cos0 * watch_point_[i].y + sin0 * watch_point_[i].x +
    //     max_weight_pose_.point.y + global_map_.info.origin.position.y;
    //     points.poses[i].position.x = x;
    //     points.poses[i].position.y = y;
    //     points.poses[i].position.z = 0;
    //     points.poses[i].orientation.w = 1;
    //     //cout << "error(" << watch_point_[i].x << "," << watch_point_[i].y
    //     << ")" << endl;
    // }
    ...
    for (int i = 0; i < pose_swarm_.size(); i++) {
        points.poses[i].position.x =
            pose_swarm_[i].point.x + global_map_.info.origin.position.x;
        points.poses[i].position.y =
            pose_swarm_[i].point.y + global_map_.info.origin.position.y;
        points.poses[i].position.z = 0;

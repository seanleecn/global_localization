...
struct Entry {
    Entry(double const& prob, size_t const& x0, size_t const& x1,
          size_t const& y0, size_t const& y1, int const& res,
          int const& _poseindex)
        : probability(prob),
          xt0(x0),
          xt1(x1),
          yt0(y0),
...
// 从占据网格地图，生成[概率机器人中文版p127页]中的提到的似然域地图
class LikelyHoodField {
   public:
    LikelyHoodField(map_t* map, double const& _sigma_hit, double const& _z_rand,
                    double const& _z_hit, double const& _max_range)
        : occupancyGrid(map),
    ...
    void computeEntry(size_t xt0, size_t xt1, size_t yt0, size_t yt1,
                      Entry const& old);
    ...
    // 计算激光雷达扫描数据的末端点，就是根据雷达的位姿和激光束的角度长度计算
    // 这个好像没有实现？没用到
   ...
   private:
    ...
    double z_hit;
    double max_range;
    /*  A list of fieldmaps, each fieldmap coarser than the one before. The
       first entry is the likelihood field with resolution 1, while each
       subsequenting fieldmap has half the resolution. An entry in a fieldmap is
       the maximum over a 3x3 grid of entries in the fieldmap with the higher
       resolution. For more information see the paper "M3RSM: Many-toMany
       Multi-Resolution Scan Matching". */
    // 参考文献 M3RSM: Many-toMany Multi-Resolution Scan Matching,
    // 这篇文章在cartographer的参考文献[1]
    // 似然域中包含了很多似然域地图
    ...
    // 用一个优先队列保存entry(条目)，队列头部是概率值最大的entry
    ...
    double distanceToNearestObstacle(int const& pose_x,
                                     int const& pose_y) const;
};
